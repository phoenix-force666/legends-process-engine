...
public class MyProcessController extends BaseController {
	...
	@Autowired
	private IMyProcessService myProcessService;

	@ApiOperation(value = "我的申请", notes = "我的申请")
	@GetMapping("apply/list")
	public TableDataInfo applyList(@RequestHeader(CacheConstants.DETAILS_USERNAME) String userName, MyProcessReq myProcessReq) throws Exception{
		log.info("userName:{}",userName);
		PageDomain pageDomain=TableSupport.getPageDomain();
		log.info(JSON.toJSONString(pageDomain));
		List<MyProcessVO> list=myProcessService.applyList(userName,myProcessReq);
		return getDataTable(list);
	}

	@GetMapping("approve/list")
	@ApiOperation(value = "待办事项", notes = "待办事项")
	public TableDataInfo  approveList(@RequestHeader(CacheConstants.DETAILS_USERNAME) String userName, MyProcessReq myProcessReq){
		log.info("userName:{}",userName);
		PageDomain pageDomain=TableSupport.getPageDomain();
		log.info(JSON.toJSONString(pageDomain));
		List<MyProcessVO> list=myProcessService.approveList(userName,myProcessReq);
		return getDataTable(list);
	}
