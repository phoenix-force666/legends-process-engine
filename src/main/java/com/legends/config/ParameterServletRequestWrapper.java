...
public class ParameterServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public ParameterServletRequestWrapper(HttpServletRequest request) {
        ...
        if(StringUtil.isNullOrEmpty(stringBuilder)){
            body=null;
            return;
        }
        ...
        String reqBody;
        try {
            ComReq comReq=JSON.parseObject(stringBuilder.toString(), ComReq.class);
            reqBody=JSON.toJSONString(comReq.getData());
        } catch (JSONException ex) {
            reqBody= stringBuilder.toString();
        }
        body=reqBody;
    }
